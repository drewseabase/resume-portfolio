...
export default function Hero(){
    ...
    useEffect(() =>{
        ...
        if(!waveContainer || !waveGroup) return;

        //Select 3 translated wave layers (top/middle/bottom)
        const waves = Array.from(waveGroup.querySelectorAll('g[transform^="translate"]'));
        if(waves.length < 3) return;

        //Prefure reduced motion
        ...
        function update(){
            ...
            const p = Math.min(latestScrollY / vh, 1);
            ...
            // Top wave slides left
            waves[0].setAttribute("transform", `translate(${base[0].x + -p * 2000}, ${base[0].y})`);
            waves[0].style.opacity = String(fade);

            // Middle wave slides right
            waves[1].setAttribute("transform", `translate(${base[1].x + p * 2000}, ${base[1].y})`);
            waves[1].style.opacity = String(fade);

            // Bottom wave rises up
            waves[2].setAttribute("transform", `translate(${base[2].x}, ${base[2].y + -p * 800})`);
            waves[2].style.opacity = "1";
            
            //Hide container once past hero(avoid display thrash)
            if(p>=1 && !hidden){
                hidden = true;
    ...
    return(
        <section className = "wave-section" id="top">
            <div className="wave-container" id="waveContainer" ref={waveContainerRef}>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1920 1080" preserveAspectRatio="xMidYMid slice">
                    <g id="waveGroup" ref={waveGroupRef}>
                        <rect width="1920" height="1080" fill="rgb(169, 219, 184)"></rect>

                        {/* First Wave (Top) - Name on curved path */}
                        <g transform="translate(0, 0)">
                            <path
                                fill="rgb(115, 148, 124)"
                                fillOpacity="1"
